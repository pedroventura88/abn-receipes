...
public class RecipeServiceTest {
    ...
    void should_get_all_available_recipes() {
        doReturn(new PageImpl<>(mockData.getRecipes())).when(repository).findAll(any(), any(Pageable.class));
    ...
    private Page<ReceipeEntity> pageServing4AndHavePotatoes() {
        List<ReceipeEntity> recipes = mockData.getRecipes().stream()
    ...
    private Page<ReceipeEntity> pageAllVegetarians() {
        List<ReceipeEntity> receipes = mockData.getRecipes().stream().filter(recipe -> recipe.getIsVegetarian().equals(true)).collect(Collectors.toList());
